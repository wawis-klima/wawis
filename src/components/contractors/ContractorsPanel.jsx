...
import ClientVoiceInput, { VoiceFieldButton } from '../voice/ClientVoiceInput.jsx';
...
function getContractorAddressLines(contractor) {
  ...
  const primaryLabel = primary ? formatContractorAddress(primary) : '';
  return [primaryLabel, addresses.length > 1 ? `+${addresses.length - 1} adres${addresses.length - 1 === 1 ? '' : 'y'}` : ''].filter(Boolean);
}

function getCompositeSortValue(contractor) {
  const addressText = normalizeContractorAddresses(contractor)
    .map((address) => `${address.label || ''} ${address.street || ''} ${address.city || ''}`)
    .join(' ');
  return [
...
function getContractorJobClientName(job) {
  return job?.client || job?.client_name || job?.contractor_name || job?.title || 'Bez klienta';
}

function getContractorJobAddress(job) {
  return [job?.city, job?.street || job?.address || job?.location].filter(Boolean).join(', ');
}

function getContractorJobStatus(job) {
  return job?.status || job?.job_status || job?.stage || 'Brak statusu';
}
...
export default function ContractorsPanel({ supabase, isAdmin, refreshAll, jobs = [], requestedContractorId = null }) {
  ...
  const selectedContractorJobs = useMemo(() => {
    ...
    const selectedName = normalizeContractorText(selectedContractor.company_name);
    const fallbackJobIds = new Set((selectedContractor.source_job_ids || []).map((id) => String(id)));
    return (jobs || [])
      .filter((job) => {
        if (fallbackJobIds.size && fallbackJobIds.has(String(job?.id || ''))) return true;
        if (selectedContractor?.id && String(job?.contractor_id || '') === String(selectedContractor.id)) return true;
        if (!selectedName) return false;
        return normalizeContractorText(getContractorJobClientName(job)) === selectedName;
      })
      .sort((left, right) => new Date(right?.installation_date || right?.created_at || 0) - new Date(left?.installation_date || left?.created_at || 0));
  }, [jobs, selectedContractor]);

  const selectedContractorSmsRows = useMemo(() => {
    return selectedContractorJobs
      .filter((job) => job?.last_sms_sent_at || job?.last_sms_status || job?.last_sms_error)
      .slice(0, 8);
  }, [selectedContractorJobs]);
  ...
  function updateContractorAddress(addressId, field, value) {
    setForm((prev) => ({
      ...prev,
      addresses: (prev.addresses || []).map((address) => (
        String(address.id) === String(addressId) ? { ...address, [field]: value } : address
      )),
    }));
  }

  function addContractorAddress() {
    setForm((prev) => ({
      ...prev,
      addresses: [
        ...(prev.addresses || []),
        createEmptyContractorAddress({
          label: `Adres ${(prev.addresses || []).length + 1}`,
          is_primary: !(prev.addresses || []).length,
        }),
      ],
    }));
  }

  function setPrimaryContractorAddress(addressId) {
    setForm((prev) => ({
      ...prev,
      addresses: (prev.addresses || []).map((address) => ({
        ...address,
        is_primary: String(address.id) === String(addressId),
      })),
    }));
  }

  function removeContractorAddress(addressId) {
    setForm((prev) => {
      const remaining = (prev.addresses || []).filter((address) => String(address.id) !== String(addressId));
      if (!remaining.length) return { ...prev, addresses: [createEmptyContractorAddress()] };
      if (!remaining.some((address) => address.is_primary)) remaining[0] = { ...remaining[0], is_primary: true };
      return { ...prev, addresses: remaining };
    });
  }

  async function handleSave() {
    const incompleteAddress = (form.addresses || []).find((address) => {
      const hasCity = String(address?.city || '').trim();
      const hasStreet = String(address?.street || '').trim();
      return Boolean(hasCity) !== Boolean(hasStreet);
    });
    if (incompleteAddress) {
      setErrorMessage('Każdy dodany adres musi mieć wpisane miasto oraz ulicę z numerem.');
      setInfoMessage('');
      return;
    }
  ...
  return (
    <div className="contractorsModulePage desktopModuleShell" data-contractor-deep-link="10.00">
      ...
      {activeView === 'list' ? (
        <>
          ...
          <div className="contractorsSplitWorkspace">
            <section className="card contractorsTableCard contractorsSplitListPane">
              <div className="contractorsSectionHeader contractorsSplitStickyHeader">
                <div>
                    ...
                    <table className="jobTable contractorsTable" aria-label="Tabela kontrahentów">
                      ...
                      <tbody>
                        {visibleContractors.map((contractor) => {
                          ...
                          const isSelected = String(selectedId || '') === String(contractor.id || '') && detailsOpen;
                          return (
                            <tr key={contractor.id || contractor.company_name} className={isSelected ? 'selected contractorsSelectedRow' : ''} aria-selected={isSelected ? 'true' : 'false'}>
                              <td>
            ...
            <aside className={`contractorsSidePanel card premiumCard ${selectedContractor && detailsOpen ? 'isOpen' : 'isEmpty'}`} aria-label="Panel szczegółów kontrahenta">
              {selectedContractor && detailsOpen ? (
                <>
                  <div className="contractorsSideStickyBar">
                    <div>
                    ...
                    <div className="contractorsSideActions">
                      <button type="button" className="btn" onClick={() => handleStartEdit(selectedContractor)}>Edytuj</button>
                      <button type="button" className="btn ghostBtn" onClick={() => setDetailsOpen(false)}>Zamknij</button>
                    </div>
                  </div>

                  <div className="contractorsSideContent">
                    <section className="contractorsDetailCard">
                      <h4>Dane klienta</h4>
                      <div className="contractorsDetailGrid">
                        ...
                        <DetailRow label="Status" value={selectedContractor.is_active ? 'Aktywny' : 'Nieaktywny'} icon={<IconCheckCircle />} />
                      </div>
                    </section>

                    <section className="contractorsDetailCard">
                      <h4>Adresy</h4>
                      {normalizeContractorAddresses(selectedContractor).length ? (
                        <div className="contractorAddressesPreview" role="list">
                          {normalizeContractorAddresses(selectedContractor).map((address) => (
                            <div key={address.id} className="contractorAddressPreviewItem" role="listitem">
                              <div>
                                <strong>{address.label || 'Adres'}</strong>
                                {address.is_primary ? <span className="contractorAddressPrimaryBadge">Główny</span> : null}
                              </div>
                              <span>{formatContractorAddress(address) || 'Brak danych adresowych'}</span>
                              {address.notes ? <small>{address.notes}</small> : null}
                            </div>
                          ))}
                        </div>
                      ) : <div className="contractorsSideEmpty">Brak zapisanych adresów.</div>}
                    </section>
                    ...
                    <section className="contractorsDetailCard">
                      <h4>Powiązane montaże</h4>
                      {selectedContractorJobs.length ? (
                        <div className="contractorsMiniList" role="list">
                          {selectedContractorJobs.slice(0, 8).map((job) => (
                            <div key={job.id || `${getContractorJobClientName(job)}-${job.installation_date}`} className="contractorsMiniItem" role="listitem">
                              <strong>{formatContractorDate(job.installation_date)}</strong>
                              <span>{getContractorJobStatus(job)} • {getContractorJobAddress(job) || 'Brak adresu'}</span>
                            </div>
                          ))}
                          {selectedContractorJobs.length > 8 ? <div className="contractorsMiniMore">+{selectedContractorJobs.length - 8} starszych montaży</div> : null}
                        </div>
                      ) : <div className="contractorsSideEmpty">Brak powiązanych montaży.</div>}
                    </section>
                        ...
                        <div className="contractorDevicesList" role="list" aria-label="Urządzenia powiązane z kontrahentem">
                          {selectedContractorDevices.map((device) => (
                            <div key={device.id} className="contractorDeviceItem" role="listitem">
                              ...
                              <span>{device.serial_number || 'Brak numeru seryjnego'}</span>
                              <span>{[device.installation_date ? `Montaż: ${formatContractorDate(device.installation_date)}` : '', device.city, device.street, device.status ? `Status: ${device.status}` : ''].filter(Boolean).join(' • ') || 'Brak dodatkowych danych'}</span>
                              <button type="button" className="btn ghostBtn contractorsImportOpenBtn" onClick={() => handleOpenDeviceEditor(device)}>Edytuj urządzenie</button>
                            </div>
                          ))}
                        </div>
                      ) : <div className="contractorsSideEmpty">Brak urządzeń przypiętych do tego kontrahenta.</div>}
                    </section>

                    <section className="contractorsDetailCard">
                      <h4>Historia SMS</h4>
                      {selectedContractorSmsRows.length ? (
                        <div className="contractorsMiniList" role="list">
                          {selectedContractorSmsRows.map((job) => (
                            <div key={`sms-${job.id}`} className="contractorsMiniItem" role="listitem">
                              <strong>{job.last_sms_sent_at ? formatContractorDate(job.last_sms_sent_at) : 'Brak daty'}</strong>
                              <span>{job.last_sms_status || 'Status nieznany'}{job.last_sms_error ? ` • ${job.last_sms_error}` : ''}</span>
                            </div>
                          ))}
                        </div>
                      ) : <div className="contractorsSideEmpty">Brak ostatnich wpisów SMS w danych tego kontrahenta.</div>}
                    </section>

                    {selectedContractor.notes ? (
                      <section className="contractorsDetailCard">
                        <h4>Notatki</h4>
                        <p className="contractorsSideNote">{selectedContractor.notes}</p>
                      </section>
                    ) : null}

                    <section className="contractorsDetailCard contractorsDetailCardDanger">
                      <h4>Zarządzanie</h4>
                      <button type="button" className="btn dangerGhostBtn" disabled={deleteBusy || saveBusy} onClick={() => void handleDeleteById(selectedContractor)}>Usuń kontrahenta</button>
                    </section>
                  </div>
                </>
              ) : (
                <div className="contractorsSidePlaceholder">
                  <div className="sectionPill">Podgląd klienta</div>
                  <h3>Wybierz kontrahenta z tabeli</h3>
                  <p>Po kliknięciu klienta zobaczysz tutaj dane, adres, kontakt, powiązane montaże, urządzenia i ostatnią historię SMS.</p>
                </div>
              )}
            </aside>
          </div>
      ...
      {activeView === 'new' || activeView === 'edit' ? (
        <section className="card contractorsEditorCard contractorsEditorCardStacked">
          ...
          <div className="contractorsFormGrid">
            ...
            <div className="contractorsField contractorsFieldFull contractorAddressesEditor">
              <div className="contractorAddressesEditorHeader">
                <div>
                  <span>Adresy klienta</span>
                  <small>Jeden klient może mieć kilka lokalizacji. Adres główny jest podpowiadany jako pierwszy przy tworzeniu montażu.</small>
                </div>
                <button type="button" className="btn secondary" onClick={addContractorAddress}>+ Dodaj adres</button>
              </div>
              <div className="contractorAddressRows">
                {(form.addresses || []).map((address, index) => (
                  <div className="contractorAddressEditorRow" key={address.id || `address-${index}`}>
                    <div className="contractorAddressEditorTop">
                      <strong>Adres {index + 1}</strong>
                      <label className="contractorAddressPrimaryChoice">
                        <input type="radio" name="primary-contractor-address" checked={!!address.is_primary} onChange={() => setPrimaryContractorAddress(address.id)} />
                        <span>Główny</span>
                      </label>
                      {(form.addresses || []).length > 1 ? (
                        <button type="button" className="fieldClearBtn" onClick={() => removeContractorAddress(address.id)}>Usuń</button>
                      ) : null}
                    </div>
                    <div className="contractorAddressEditorGrid">
                      <label className="contractorsField">
                        <span>Nazwa adresu</span>
                        <input className="input" value={address.label || ''} onChange={(event) => updateContractorAddress(address.id, 'label', event.target.value)} placeholder="np. Dom, Firma, Magazyn" />
                      </label>
                      <label className="contractorsField">
                        <span>Miasto</span>
                        <div className="voiceFieldRow">
                          <input className="input" value={address.city || ''} onChange={(event) => updateContractorAddress(address.id, 'city', event.target.value)} placeholder="Miasto" />
                          <VoiceFieldButton label={`Miejscowość adresu ${index + 1}`} onValue={(value) => updateContractorAddress(address.id, 'city', value)} disabled={saveBusy} />
                        </div>
                      </label>
                      <label className="contractorsField contractorsFieldFull">
                        <span>Ulica i numer</span>
                        <div className="voiceFieldRow">
                          <input className="input" value={address.street || ''} onChange={(event) => updateContractorAddress(address.id, 'street', event.target.value)} placeholder="Ulica i numer" />
                          <VoiceFieldButton label={`Ulica i numer adresu ${index + 1}`} onValue={(value) => updateContractorAddress(address.id, 'street', value)} disabled={saveBusy} />
                        </div>
                      </label>
                      <label className="contractorsField contractorsFieldFull">
                        <span>Notatka do adresu (opcjonalnie)</span>
                        <input className="input" value={address.notes || ''} onChange={(event) => updateContractorAddress(address.id, 'notes', event.target.value)} placeholder="np. wjazd od tyłu, kontakt z ochroną" />
                      </label>
                    </div>
                  </div>
                ))}
      ...
      <AppModal open={false} onClose={() => setDetailsOpen(false)} contentClassName="card contractorsDetailsModal contractorsDetailsDialog">
        {selectedContractor ? (
          <>
