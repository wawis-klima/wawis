...
function getDeviceAddress(device) {
  const noteAddress = isAddressLikeNote(device?.notes)
    ? cleanAddressLikeNote(device?.notes, device?.contractor_city)
    : '';
  return [device?.contractor_city, noteAddress].filter(Boolean).join(', ');
}

function getDeviceIssues(device) {
  const issues = [];
  if (!String(device?.installation_date || '').trim()) issues.push('Brak daty montażu');
  if (!String(device?.serial_number || '').trim()) issues.push('Brak numeru seryjnego');
  if (!String(device?.contractor_name || '').trim()) issues.push('Brak klienta');
  if (!String(device?.contractor_phone || '').trim()) issues.push('Brak telefonu');
  if (!String(device?.source_job_id || '').trim()) issues.push('Brak powiązania z montażem');
  return issues;
}

function findRelatedDeviceJob(device, jobs = []) {
  if (!device) return null;
  const sourceJobId = String(device.source_job_id || '').trim();
  if (sourceJobId) {
    const direct = jobs.find((job) => String(job?.id || '') === sourceJobId);
    if (direct) return direct;
  }
  const contractorId = String(device.contractor_id || '').trim();
  const contractorName = normalizeText(device.contractor_name);
  const serial = normalizeText(device.serial_number);
  const model = normalizeText(device.model);
  return jobs.find((job) => {
    if (contractorId && String(job?.contractor_id || '') === contractorId) return true;
    const jobName = normalizeText(job?.client || job?.client_name || job?.contractor_name || job?.title);
    if (contractorName && jobName === contractorName) return true;
    const jobSerials = [job?.device_serial_number, job?.serial_number, job?.outdoor_serial_number, job?.indoor_serial_number]
      .map(normalizeText)
      .filter(Boolean);
    if (serial && jobSerials.includes(serial)) return true;
    const jobModel = normalizeText(job?.device_model || job?.model || job?.air_conditioner_model);
    return Boolean(model && jobModel && model === jobModel && contractorName && jobName === contractorName);
  }) || null;
}

function getDeviceJobLabel(job) {
  if (!job) return 'Brak powiązanego montażu';
  const client = job.client || job.client_name || job.contractor_name || job.title || 'Montaż';
  const date = job.installation_date ? formatInstallationDate(job.installation_date) : 'brak daty';
  return `${client} • ${date}`;
}
...
export default function DevicesPanel({ supabase, jobs = [], isAdmin, refreshAll, contractors = [], requestedDeviceId = null }) {
  ...
  useEffect(() => {
    ...
    if (!requestedId || !devices.length) return;
    const requestedDevice = devices.find((device) => String(device?.id || '') === requestedId);
    if (!requestedDevice) return;
    setSearch('');
    setStatusFilter('all');
    setSelectedClientDevice(requestedDevice);
  ...
  const selectedDevice = selectedClientDevice ? normalizeDeviceRecord(selectedClientDevice) : null;
  const selectedDeviceIssues = useMemo(() => getDeviceIssues(selectedDevice), [selectedDevice]);
  const selectedDeviceRelatedJob = useMemo(() => findRelatedDeviceJob(selectedDevice, jobs), [selectedDevice, jobs]);
  ...
  return (
    <div className="devicesModulePage desktopModuleShell">
      ...
      <div className="devicesSplitWorkspace">
        <section className="card devicesTableCard desktopDataCard devicesSplitListPane">
          <div className="devicesSectionHeader devicesSplitStickyHeader">
                ...
                <table className="jobTable devicesTable" aria-label="Tabela urządzeń">
                  ...
                  <tbody>
                    {visibleDevices.map((device) => {
                      ...
                      const deviceIssues = getDeviceIssues(device);
                      return (
                        <tr key={device.id} className={isSelected ? 'devicesSelectedRow' : ''} aria-selected={isSelected ? 'true' : 'false'}>
                          <td>
                          ...
                          <td>
                            <button type="button" className="devicesRowDetailsButton" onClick={() => setSelectedClientDevice(device)} title="Pokaż szczegóły urządzenia">
                              ...
                              {deviceIssues.length ? <span className="devicesIssueInline">{deviceIssues.length} braków</span> : null}
                            </button>
                          </td>
        ...
        <aside className={`devicesSidePanel card premiumCard ${selectedDevice ? 'isOpen' : 'isEmpty'}`} aria-label="Panel szczegółów urządzenia">
          {selectedDevice ? (
            <>
              <div className="devicesSideStickyBar">
                <div>
                  <div className="sectionPill">Szczegóły urządzenia</div>
                  <h3>{selectedDevice.model || 'Brak modelu'}</h3>
                  <p>{selectedDevice.contractor_name || 'Bez klienta'}</p>
                </div>
                <div className="devicesSideActions">
                  <button type="button" className="btn" onClick={() => openDeviceEditor(selectedDevice)}>Edytuj</button>
                  <button type="button" className="btn ghostBtn" onClick={() => setSelectedClientDevice(null)}>Zamknij</button>
                </div>
              </div>

              <div className="devicesSideContent">
                {selectedDeviceIssues.length ? (
                  <section className="devicesDetailCard devicesDetailCardWarning">
                    <h4>Braki do uzupełnienia</h4>
                    <div className="devicesIssuesList">
                      {selectedDeviceIssues.map((issue) => <span key={issue}>{issue}</span>)}
                    </div>
                  </section>
                ) : null}

                <section className="devicesDetailCard">
                  <h4>Urządzenie</h4>
                  <div className="contractorsDetailsGrid devicesDetailsGridTight">
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconCheckCircle /><span>Model</span></span><strong>{selectedDevice.model || 'Brak modelu'}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconCheckCircle /><span>Numer seryjny</span></span><strong>{selectedDevice.serial_number || 'Brak numeru seryjnego'}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconCalendar /><span>Data montażu</span></span><strong>{formatInstallationDate(selectedDevice.installation_date)}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconCheckCircle /><span>Status</span></span><strong><span className={`badge ${getStatusBadgeClass(selectedDevice.status)} devicesStatusBadge`}>{STATUS_LABELS[normalizeDeviceStatus(selectedDevice.status)]}</span></strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconCheckCircle /><span>Źródło</span></span><strong>{getDeviceSourceLabel(selectedDevice.source_kind)}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconMapPin /><span>Notatka/adres</span></span><strong>{getDeviceMeta(selectedDevice) || getDeviceAddress(selectedDevice) || '—'}</strong></div>
                  </div>
                </section>

                <section className="devicesDetailCard">
                  <h4>Powiązany klient</h4>
                  <div className="contractorsDetailsGrid devicesDetailsGridTight">
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconUsers /><span>Klient</span></span><strong>{selectedDevice.contractor_name || 'Bez klienta'}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconPhone /><span>Telefon</span></span><strong>{selectedDevice.contractor_phone || 'Brak telefonu'}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconMail /><span>E-mail</span></span><strong>{selectedDevice.contractor_email || 'Brak e-maila'}</strong></div>
                    <div className="contractorsPreviewItem"><span className="infoLabelWithIcon"><IconMapPin /><span>Adres</span></span><strong>{getDeviceAddress(selectedDevice) || selectedDevice.contractor_city || 'Brak adresu'}</strong></div>
                  </div>
                </section>

                <section className="devicesDetailCard">
                  <h4>Powiązany montaż</h4>
                  <div className="devicesRelatedJobBox">
                    <strong>{getDeviceJobLabel(selectedDeviceRelatedJob)}</strong>
                    <span>{selectedDeviceRelatedJob ? [selectedDeviceRelatedJob.status, selectedDeviceRelatedJob.city, selectedDeviceRelatedJob.street].filter(Boolean).join(' • ') : 'Uzupełnij powiązanie, żeby łatwiej wracać do montażu.'}</span>
                  </div>
                </section>

                <section className="devicesDetailCard">
                  <h4>Historia SMS</h4>
                  <button type="button" className="btn secondary" onClick={() => void openDeviceHistory(selectedDevice)} disabled={historyBusy}>Otwórz historię SMS</button>
                  <p className="devicesDetailHint">Historia nadal otwiera się w osobnym oknie, żeby nie mieszać listy logów z podglądem urządzenia.</p>
                </section>
              </div>
            </>
          ) : (
            <div className="devicesSidePlaceholder">
              <div className="sectionPill">Podgląd urządzenia</div>
              <h3>Wybierz urządzenie z tabeli</h3>
              <p>Po kliknięciu modelu lub klienta zobaczysz tutaj dane urządzenia, klienta, powiązany montaż, braki i skrót do historii SMS.</p>
            </div>
          )}
        </aside>
      </div>
      ...
      <AppModal open={false} onClose={() => setSelectedClientDevice(null)} contentClassName="card contractorsDetailsDialog devicesClientDetailsDialog">
