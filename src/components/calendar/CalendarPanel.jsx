...
export default function CalendarPanel({ jobs = [], focusedDateKey = '', onOpenJob }) {
  ...
  return (
    <div className="calendarModulePage calendarModulePageWide">
      ...
      <section className="calendarContentGrid calendarSplitWorkspace calendarDesktopWideSection">
        ...
        <aside className="calendarDetailsCard card premiumCard calendarSplitDetailsPanel">
          <div className="calendarDetailsHeader calendarDetailsStickyHeader">
            <span>Wybrany dzień</span>
            <strong>{selectedDateKey}</strong>
          </div>
          {selectedJobs.length ? (
            <div className="calendarJobsList calendarJobsListCards">
              {selectedJobs.map((job) => (
                <button type="button" key={job.id || `${selectedDateKey}-${getClientName(job)}`} className="calendarJobDetailsItem calendarJobDetailsCard" onClick={() => onOpenJob?.(job)}>
