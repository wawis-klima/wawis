...
export function useRealtimeRefresh({
  ...
  useEffect(() => {
    ...
    const scheduleRefreshIfStale = (options = {}) => {
      const ageMs = Date.now() - Number(lastGlobalRefreshAttemptAtRef.current || 0);
      if (ageMs < RESUME_GLOBAL_REFRESH_MIN_AGE_MS) return;
    ...
    const channel = supabase
      ...
      // Stan notifications nie jest renderowany w mobilnym UI; nie ma powodu
      // pobierać przez niego całej listy montaży.
    ...
    const refreshVisibleSelection = () => {
      const selectedId = normalizeId(selectedJobIdRef.current);
      if (selectedId) {
        scheduleJobSummaryReload(selectedId, { immediate: true });
        scheduleSelectedDetailsReload(selectedId, { immediate: true });
      }
      // Po krótkim przełączeniu aplikacji nie pobieramy całej bazy od nowa.
      // Pełna lista odświeża się dopiero po dłuższej przerwie.
      scheduleRefreshIfStale({ immediate: true });
    };
