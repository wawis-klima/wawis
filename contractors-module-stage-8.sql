...
create index if not exists contractors_name_idx on public.contractors (lower(company_name));
create index if not exists contractors_nip_idx on public.contractors (nip);
...
drop function if exists public.admin_upsert_contractor(uuid, text, text, text, text, text, text, text, boolean);
...
alter table public.jobs add column if not exists contractor_id uuid references public.contractors(id) on delete set null;
...
grant execute on function public.admin_upsert_contractor(uuid, text, text, text, text, text, text, text, text, boolean) to authenticated;
