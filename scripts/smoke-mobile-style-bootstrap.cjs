...
const mobileCssPath = path.join(root, 'src/mobile791/mobile.css');
...
assert.match(
  main,
  /Promise\.all\([\s\S]*import\('\.\/mobile791\/App\.jsx'\)[\s\S]*import\('\.\/mobile791\/styles\.css'\)[\s\S]*import\('\.\/mobile791\/styles\/desktop-jobs-table\.css'\)/,
  'Mobilna aplikacja, bazowy CSS i CSS tabeli muszą być ładowane razem przed renderem.'
);
assert.match(
  main,
  /if \(useMobile791\) \{[\s\S]*await import\('\.\/mobile791\/mobile\.css'\)[\s\S]*\}[\s\S]*diagnosticsModule\.installDiagnosticConsoleCapture\(\)[\s\S]*const App = appModule\.default[\s\S]*ReactDOM\.createRoot/,
  'Mobilne nakładki CSS muszą zostać doładowane przed diagnostyką i renderem React.'
);
...
assert.match(mobileCss, /@import '\.\/styles\.css';/, 'Docelowy mobile.css musi importować bazowy arkusz mobile.');
...
assert.match(mobileBaseCss, /body\s*\{[^}]*font-family:/, 'Mobilny CSS musi ustawiać font całej aplikacji.');
assert.match(mobileBaseCss, /\.page\s*\{/, 'Mobilny CSS musi zawierać podstawowy układ strony.');
