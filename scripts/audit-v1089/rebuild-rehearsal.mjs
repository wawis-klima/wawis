...
await db.exec(fs.readFileSync(new URL('supabase/rebuild/verify_audit_v1089.sql',root),'utf8'));await db.exec(fs.readFileSync(new URL('scripts/audit-v1089/fixtures/backend-restore.sql',root),'utf8'));console.log('Local dependency rehearsal + catalog checks + delete/restore history PASS; this local run alone does not verify staging; see separate staging evidence');
