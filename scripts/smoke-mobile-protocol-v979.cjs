...
const reviewQrModule = read('src', 'mobile791', 'modules', 'protocol-review-qr.js');
...
assert.match(modal, /openStoredJobProtocolPdfPreview/);
assert.doesNotMatch(modal, /shareStoredJobProtocol\(\{ supabase, record: savedRecord/);
...
assert.match(storageModule, /const PDF_MIME_TYPE = "application\/pdf"/);
...
assert.match(storageModule, /window\.open\("", "_blank"\)/);
assert.match(storageModule, /previewWindow\.location\.replace\(signedUrl\)/);
assert.match(storageModule, /method: "pdf-native-preview"/);
assert.doesNotMatch(storageModule, /mailto:|EMAIL_LINK_TTL_SECONDS/);
...
assert.match(reviewQrModule, /PROTOCOL_REVIEW_URL/);
assert.match(reviewQrModule, /search\.google\.com\/local\/writereview\?placeid=ChIJ5R1WPK4jF0cRPUfNSCXd1d4/);
assert.match(reviewQrModule, /PROTOCOL_REVIEW_QR_MATRIX/);
assert.match(pdfModule, /drawReviewCard\(doc, PAGE_WIDTH - OUTER_MARGIN - 136, 7, 136, 60\)/);
assert.doesNotMatch(pdfModule, /doc\.text\(\`ID \$\{data\.shortJobId\}\`/);
assert.doesNotMatch(pdfModule, /Podpisano: \$\{data\.signedAt\}/);
assert.doesNotMatch(pdfModule, /drawField\(doc, "Status"/);
assert.doesNotMatch(pdfModule, /drawField\(doc, "Zakończył"/);
assert.doesNotMatch(pdfModule, /drawField\(doc, "Zakończono"/);
assert.match(pdfModule, /doc\.splitTextToSize\(confirmation, CONTENT_WIDTH\)/);
...
assert.match(pdfModule, /drawCard\(doc, y \+ 8, 42\)/);
...
assert.match(modal, /const SIGNATURE_MIN_WIDTH = 1\.65/);
assert.match(modal, /const SIGNATURE_MAX_WIDTH = 3\.25/);
assert.match(modal, /const SIGNATURE_BASE_WIDTH = 2\.45/);
assert.match(modal, /Math\.min\(4, Math\.max\(2, window\.devicePixelRatio \|\| 1\)\)/);
assert.match(modal, /context\.imageSmoothingQuality = "high"/);
assert.match(modal, /getCoalescedEvents/);
assert.match(modal, /quadraticCurveTo/);
assert.match(modal, /getSignatureStrokeWidth/);
assert.match(modal, /pixels\[offset\] >= 252/);
...
assert.doesNotMatch(pdfModule, /doc\.text\("STATUS"/, 'Protocol PDF must not render a nameplate status column.');
assert.doesNotMatch(pdfModule, /Zapisana w systemie|Zapisana na telefonie|Błąd wysyłania/, 'Protocol PDF module must not expose nameplate storage status.');
assert.doesNotMatch(modal, /row\.nameplate|protocolTestDeviceStatus/, 'Protocol preview must not render nameplate storage status.');
