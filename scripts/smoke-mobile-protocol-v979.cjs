...
assert.match(storageModule, /navigator\.share\(\{ files: \[pdfFile\] \}\)/);
...
assert.match(storageModule, /share-image-fallback/);
assert.match(storageModule, /pdfjs-dist\/legacy\/build\/pdf\.mjs/);
assert.match(storageModule, /return \{ method: "share-pdf", copies: 1, a4Document: true \}/);
