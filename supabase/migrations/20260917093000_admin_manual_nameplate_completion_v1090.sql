-- WAWIS 10.90 — administrator może zakończyć montaż po ręcznym
-- potwierdzeniu każdej brakującej tabliczki JZ/JW.
-- Pracownik nadal musi posiadać rzeczywiste zdjęcia tabliczek.
  ...
  loop
    if not exists (
      ...
      and exists (
        ...
        where mv.job_id = p_job_id
          and mv.device_index = v_required.device_index
          and lower(pg_catalog.btrim(coalesce(mv.unit_ref, ''))) = v_required.unit_ref
      )
    ) then
...
comment on function private.assert_job_nameplates_complete(uuid, text, text) is
  'WAWIS 10.90: pracownik wymaga fizycznych zdjęć JZ/JW; administrator może zastąpić brakujące zdjęcia jawnie zapisanymi ręcznymi potwierdzeniami.';
