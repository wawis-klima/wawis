...
begin
  ...
  insert into public.jobs select * from jsonb_populate_record(null::public.jobs,(r.snapshot->'jobs') || jsonb_build_object('status','W trakcie'));
  ...
  update public.jobs set status=r.snapshot->'jobs'->>'status' where id=r.job_id;
  -- Completion trigger stamps this transition; restore original metadata afterwards.
