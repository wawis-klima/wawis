...
begin
  ...
  with remaining as (
    select
      p.id,
      p.storage_path as old_storage_path,
      coalesce(
        nullif(p.device_index, 0),
        nullif(substring(coalesce(nullif(p.storage_path, ''), p.image_url, '') from '/nameplates/device-([0-9]+)_'), '')::integer
      ) as old_device_index,
    ...
    select
      r.*,
      case
        when r.old_device_index > p_device_index and coalesce(r.old_storage_path, '') <> '' then regexp_replace(
          r.old_storage_path,
          '/nameplates/device-[0-9]+_',
          '/nameplates/device-' || (r.old_device_index - 1)::text || '_'
        )
        else r.old_storage_path
      end as legacy_moved_storage_path
    from remaining r
  )
  ...
  set
    ...
    storage_path = case
      when r.old_device_index > p_device_index
        and coalesce(r.old_storage_path, '') <> ''
        and not exists (
          select 1 from storage.objects o
          where o.bucket_id = 'job-photos' and o.name = r.old_storage_path
        )
        and exists (
          select 1 from storage.objects o
          where o.bucket_id = 'job-photos' and o.name = r.legacy_moved_storage_path
        )
      then r.legacy_moved_storage_path
      else p.storage_path
    end
  from prepared r
